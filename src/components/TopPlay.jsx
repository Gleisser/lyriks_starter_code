      ...
      <div className="w-full flex flex-col mt-8">
        ...
        <Swiper slidesPerView="auto" spaceBetween={15} freeMode centeredSlides centeredSlidesBounds modules={[FreeMode]} className="mt-4">
          {topPlays?.slice(0, 5).map((artist) => (
            <SwiperSlide key={artist.key} style={{ width: '25%', height: 'auto' }} className="shadow-lg animate-slideright">
              <Link to={`/artists/${artist?.artists[0].adamid}`}>
                <img alt="Name" src={artist?.images?.background} className="rounded-full w-full object-cover" />
              </Link>
            </SwiperSlide>
          ))}
        </Swiper>
